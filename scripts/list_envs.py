...
All the environments are registered in the `oceanisaaclab` extension. They start
...
with contextlib.suppress(ImportError):
    import oceanisaaclab.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~oceanisaaclab.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from oceanisaaclab.utils.preset_target import PresetTarget
...
def main():
    """Print all environments registered in `oceanisaaclab` extension."""
    ...
    if args_cli.show_presets:
        from oceanisaaclab.utils.preset_cli import enumerate_task_presets
