...
为适配本机“无扰动时脚应锁定、大扰动时允许捕获步”的验收目标，当前训练将 episode 分层：
默认 50% episode 完全关闭外力作为 nominal 双支撑基线，另外 50% 保持完整 Table V 独立扰动过程。
扰动 episode 内的力/矩幅值、开关时长和课程不变；该分层不改变奖励，也不作为观测输入。
...
- 代码修改后已通过 Python 编译和 `git diff --check`；固定 Table I 后需重新运行 64 env × 1
  PPO iteration smoke test，确认 `policy/critic/action=77/79/14`、无 traceback/NaN。
  旧 run 的 gate 指标不再适用于当前配置。
